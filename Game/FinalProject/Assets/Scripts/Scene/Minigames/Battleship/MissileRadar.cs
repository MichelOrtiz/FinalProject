...
[CreateAssetMenu(fileName="MissileRadar", menuName = "Battleship/MissileRadar")]
public class MissileRadar : MissileScript
{
    ...
    void SetColors(byte[] array)
    {
        ...
        {
            if (manager.CheckHit(array[i], false))
            {
                tiles.Find(tile => tile.numberId == array[i])?.SetTileColor(manager.flagColor);
            }
        }
